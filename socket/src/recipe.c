...
struct Supply* generate_supply_list()
{
    ...
    for(int i=0; 1; i++)
    {
        ...
        else if(state == READING_SUP_NAME && buf[i] == '"')
        {
            ...
            index = 0;
            if(is_repetitive(head, temp_sup->name)) free(temp_sup);
            else add_to_sup_list(head, temp_sup);
            state = STAGE3;
        }
        else if(state == STAGE3 && buf[i] >= '1' && buf[i] <= '9')
        {
...
{
    struct Supply* iterator = sup_head->next;

    while(iterator != NULL)
    {
        if(strcmp(iterator->name, sup_name) == 0) return TRUE;
...
{
    struct Supply* iterator = food_head->next;

    while(iterator != NULL)
    {
        if(strcmp(iterator->name, food_name) == 0) return TRUE;
        iterator = iterator->next;
    }
...
struct Supply* find_supply(char* name, struct Supply* sup_head)
{
    ...
    while(iterator != NULL)
    {
        if(strcmp(name, iterator->name) == 0) break;
        iterator = iterator->next;
    }
...
struct Food* find_food(char* name, struct Food* food_head)
{
    ...
    while(iterator != NULL)
    {
        if(strcmp(iterator->name, name) == 0) break;
        iterator = iterator->next;
    }
